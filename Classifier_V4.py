...
class Classifier:
    ...
    def __init__(self,X_feat,current_datastream,index,public_sim_sd = [],GPU_feature = True, max_sim_factor=2,min_sim_factor=2,p_value_st = 0.05):
        #Definition Headers
        self.index = index
        ...
        self.std_min_sim_factor = min_sim_factor
        self.public_sim_sd = public_sim_sd
        self.min_similarity_scaling_factor = self.std_min_sim_factor
        #self.p_value_st = p_value_st
        self.max_similarity = None
        self.allowed_similarity = None
        self.similarity_list = []
        self.max_num_fingurprint_allowed = 6
    ...
    def update(self,states,new_similarity,new_fingerprint,X_feat,current_datastream,GPU=False):
        ...
        if states == 1:
            ...
            if len(self.fingerprint_pool) >= self.max_num_similarity_allowed:
            ...
            self.min_similarity_scaling_factor =  self.std_min_sim_factor / (np.mean(self.similarity_list) / np.mean(self.public_sim_sd))
            ...
            Y_datastream = current_datastream["Y"].to_numpy(dtype = "float32")
            self.X_feat_pool.insert(0,X_feat)
            self.Y_pool.insert(0,Y_datastream)
            self.train(np.concatenate(self.X_feat_pool),np.concatenate(self.Y_pool))
            ...
            if len(self.similarity_list) >= 2:
                self.public_sim_sd.append(np.mean(self.similarity_list)) 
                self.min_similarity_scaling_factor =  self.std_min_sim_factor / (np.mean(self.similarity_list) / np.mean(self.public_sim_sd))
